...
│   ├── eval/              # 检索质量评测器
│   ├── demo/              # 生成 README 的演示素材（docs/demo.svg）
│   ├── bench/             # 规模基准（导入 / 内存 / 延迟）
...
- **向量检索走哪条路由存储决定，而 HNSW 索引在评测规模上根本不会被选中**：
  ...
  **HNSW 的近似召回损失因此在当前规模下测不出来**。
  详见 [docs/BENCHMARKS.md](docs/BENCHMARKS.md)
- **规模：导入是二次方的，检索是线性的**（实测，内存后端）：

  | 片段数 | 导入耗时 | 堆占用 | 检索 P50 |
  | --- | --- | --- | --- |
  | 1,000 | 234 ms | 13.9 MB | 0.69 ms |
  | 10,000 | **38.7 s** | 137.5 MB | 21.6 ms |
  | 20,000 | **167 s** | 275.0 MB | 48.3 ms |

  **导入每 10 倍规模涨约 40 倍**——因为每导入一篇就全量重建一次索引。
  十万片段按这个趋势要一个多小时，**在正常导入路径下不可达**。见 #81。
  内存稳定在约 14 KB/片段，检索约 2.4 µs/片段（都是线性的）。
  复现：`go run ./cmd/bench -n 10000`
